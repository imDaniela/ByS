...
class PedidosAlbaranApi {
  ...
  static Future<http.Response> searchPedido(String? query) async {
    ...
    String url = '${kDebugMode ? fakeEndPoint : GlobalConstants.apiEndPoint}get-pedido-albaran?search=${query??''}';
  ...
  static Future<http.Response> getPedido(int codigo) async {
    String url = '${kDebugMode ? fakeEndPoint : GlobalConstants.apiEndPoint}get-pedido-albaran/$codigo}';
    final response = await http.get(Uri.parse(url), headers: GlobalConstants.header());
    return response;
  }
